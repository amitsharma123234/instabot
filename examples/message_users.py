...
if deliveryMethod == 0:
    with open("messages.csv", "rU") as f:
        reader = csv.reader(f)
        for row in reader:
            print(f"Messaging {row[0]}")
            bot.send_message(row[1], row[0])
            print(f"Waiting {str(banDelay)} seconds...")
            time.sleep(banDelay)
...
with open("medialikers.txt", "w") as f:
    for users in pages_to_scrape:
        ...
        for likers in getlikers:
            f.write(likers + "\n")
    print(f"succesfully written latest medialikers of{str(pages_to_scrape)}")
...
with open("usernames.txt", "w") as f:
    for user_id in wusers:
        ...
        f.write(username + "\n")
print(f"succesfully converted  {str(wusers)}")
